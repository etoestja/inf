...
Пусть $(G(V,E)$, $c\colon V^2\to\Nz, s, t)$~--- транспортная сеть. $f_1$ и $f_2$~--- потоки в ней. Определим функцию $f\colon V^2\to \mathbb{R}$ как $f(u,v)\eqdef f_1(u,v)+f_2(u,v)$. По определению, $f$~--- поток в данной транспортной сети $\Leftrightarrow$ \begin{enumerate}
