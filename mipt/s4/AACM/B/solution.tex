...
\item $A_i^3$. Рассмотрим строки $u\eqdef t$ и $v\eqdef p^R0^{n-m}$ (при $n<m$ задача не имеет решения: образец длиннее текста). Рассмотрим их как коэффициенты многочленов (порядок: начиная от свободного члена). Рассмотрим $u* v$~--- коэффициенты произведения этих многочленов. $(u* v)_k=\sum\limits_{j=0}^{k}v_ju_{k-j}=\sum\limits_{j=0}^k\left[\begin{cases}
