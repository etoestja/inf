...
boot.ci(boot(data$Price[data$Year == 2001], statistic = mean_ind, R = 100), type=c("norm", "basic"))
boot.ci(boot(data$Price[data$Year == 2002], statistic = mean_ind, R = 100),type=c("norm", "basic"))
```

3. Воcпользуйтесь критерием Смирнова (`ks.test`). 
```{r}
ks.test(x = data$Price[data$Year == 2001], y = data$Price[data$Year == 2002])
```
