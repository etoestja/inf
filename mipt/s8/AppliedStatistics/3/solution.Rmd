...
```{r, echo = FALSE}
visualize_model <- function(m, data_ = data)
{
  par(mfrow=c(4,2))
  ...
  plot(1:dim(data_)[1], rstandard(m), xlab="i", ylab="Standardized residuals", pch=19)
  addtrend(1:dim(data_)[1], rstandard(m))
  grid()
...
Имеется квадратичная зависимость по нагрузке и кубическая по диаметру.
