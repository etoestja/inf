...
%\end{enumerate}
\section*{Задача 2}
{\em{Идея обсуждалась вместе с Владом Гончаренко, но конца не придумали.}}
\begin{enumerate}[1.]
...
\item Рассмотрим автомат $\C$, имитирующий работу двух входный автоматов $\A$ и $\B$ (такой построен в задаче 4.4). В нем $|Q^\A| \cdot |Q^\B|$ состояний. Пути до этих состояний не длиннее $|Q^\A| \cdot |Q^\B|$, поэтому, перебрав все $w\colon |w|\leqslant |Q^\A| \cdot |Q^\B|$, автомат побывает в каждой вершине. Значит, пара из конечных состояний входных автоматов после прочтения слов $\{w\}$ достигнет всех своих возможных значений.
\item Пусть автоматы одновременно принимают и не принимают слова $\{w\big||w|\leqslant|Q^\A||Q^\B|\}$. Выше доказано, что при обработке этих слов пара $(q^\A_i,q^\B_j)$, где $\delta^\A(q^\A_0,w)=q^\A_i,\delta^\B(q^\B_0,w)=q^\B_j$ примет все свои возможные значения.
\item ???
\end{enumerate}
\end{enumerate}
\section*{Задача 3}
