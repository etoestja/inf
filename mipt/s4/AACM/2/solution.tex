...
0 & 1 & (1-\lambda)
\end{array}
\right|=(1-\lambda)^3-2(1-\lambda)=(1-\lambda)\cdot(1+\lambda^2-2\lambda-2)=(1-\lambda)\cdot(\lambda^2-2\lambda-1)$. Корни квадратного уравнения $\lambda=1$ и $\lambda\in\frac{2\pm\sqrt{4\cdot 2}}{2}=1\pm\sqrt{2}$. Далее ищем собственные векторы.
...
\item Вернемся к $g_n=\vec{\xi}^TA^{n-1}\vec{\xi}=\vec{\xi}^TS\mbox{diag}(\lambda_1^{n-1},\lambda_2^{n-1},\lambda_3^{n-1})S^T\vec{\xi}=\frac{1}{2}\left[(1+\sqrt{2})^{n+1}+(1-\sqrt{2})^{n+1}\right]$
\end{enumerate}
\end{document}