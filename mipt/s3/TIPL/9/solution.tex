...
\item Докажем, что ${\rm Pref}(X\cap Y)={\rm Pref}(X)\cap{\rm Pref}(Y)$: $x\in {\rm Pref}(X\cap Y)\Leftrightarrow xy\in X\cap Y\Leftrightarrow xy\in X,xy\in Y\Leftrightarrow x\in{\rm Pref}(X),x\in {\rm Pref}(Y)\Leftrightarrow x\in {\rm Pref}(X)\cap {\rm Pref}(Y)$
\item Докажем, что $\varphi({\rm Pref}(D_n))=\varphi(D_n)$: $\varphi(\sigma)\neq\varepsilon$ только если $\sigma=]_a$. Поскольку ${\rm Pref}(D_n)\supseteq D_n$, $\varphi({\rm Pref}(D_n))\supseteq\varphi(D_n)$. Теперь докажем, что $\varphi({\rm Pref}(D_n))\subseteq\varphi(D_n)$. Пусть $x\in\varphi({\rm Pref}(D_n))$. Если $x=\varepsilon$, возьмем $w=\varepsilon\in D_n\Rightarrow x=\varphi(w)$. Если $|x|>0,\,x=x_1...x_n$, возьмем $w=[_{x_n}...[_{x_1}]_{x_1}...]_{x_n}$. Тогда
\end{enumerate}
(не дописано).
\section*{Задача 3}
