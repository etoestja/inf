...
\end{tikzpicture}\newline
Высота дерева $h=\log_4 n$, $T(n)=\sum\limits_{k=0}^{h-1}4^kf(\frac{n}{3^k})+4^hT(1)$.
Из определения $\Omega$ $\exists n_0\,\exists C>0\colon\forall n\geqslant n_0\hookrightarrow \sum\limits_{k=0}^{h-1}4^kf(\frac{n}{3^k})\geqslant Cn\sum\limits_{k=0}^{h-1}\frac{4^k}{3^k}\underset{\mbox{\tiny прогр.}}{\overset{\mbox{\tiny геом.}}{=}}Cn\frac{(4/3)^{h-1}-1}{4/3-1}$
\end{enumerate}
\subsection*{Задача 2}
\subsection*{Задача 3}
