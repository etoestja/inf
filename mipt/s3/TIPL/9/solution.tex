...
\item $P(k)\eqdef\big[\forall n\,\forall w\colon S\overset{\mbox{\tiny левый вывод}}{\underset{\mbox{\tiny k шагов}}{\Rightarrow^*}} w\equiv u[r_0Y_1r_1]...[r_{n-1}Y_nr_n]\hookrightarrow(q_0,u,Z_0)\vdash^*(r_0,\varepsilon,Y_1...Y_n)\big]$\begin{enumerate}
...
\item Пусть $P(k)$. Рассмотрим левый вывод длины $k+1$: $S\Rightarrow^*y\equiv u[r_0Y_1r_1][r_1Y_2r_2]...[r_{n-1}Y_nr_n]$. Пусть начальная часть этого вывода длины $k$ имеет вид $S\Rightarrow^*x\equiv u_l[s_0Z_1s_1][s_1Z_2s_2]...[s_{m-1}Z_ms_m]$. На последнем, $k+1$-м шаге был раскрыт первый нетерминал $[s_0Z_1s_1]\to z\equiv u_r[t_0W_1t_1][t_1W_2t_2]...[t_{l-1}W_lt_l]$.\newline
...
\end{enumerate}
\item Пусть $\underline{w\in L(G)}\Rightarrow\exists k\colon S\overset{\mbox{\tiny левый вывод}}{\underset{\mbox{\tiny n шагов}}{\Rightarrow^*}} w\in\Sigma^*\overset{P(n)}{\Rightarrow}(q_0,w,Z_0)\vdash^*(q_0,\varepsilon,\varepsilon)\Rightarrow \underline{w\in L(M)}$
\end{enumerate}
