...
\prp Рассмотрим класс языков $\C$, сводимых к языку $\SAT$ в следующем смысле: для каждого языка $L$ из $\C$ существует машина Тьюринга $M_{\SAT}$, которая распознаёт язык $L$, причём машина $M_{\SAT}$ делает не более одного запроса к оракулу. Доказать, что класс $\C = \NP\cup\coNP$.
