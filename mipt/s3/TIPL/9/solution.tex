...
\end{enumerate}
\end{enumerate}
\item $q=q_2$. В $q_2$ есть переходы только из $q_1$, в $q_2$ прочитываются $a,b$. $\ref{1.2q1}\Rightarrow w=xcy,\,|x|\neq 0,\,x,y\in\{a,b\}^*$. При переходе в $q_2$ прочитывается символ, поэтому $|y|>0$. Аналогично $\ref{1.2q1}$ выделим общую часть $x^R=\tau x_1,y=\tau y_1$. Аналогично $\ref{1.2q1}$ ($|x|>0$) получаем $(q_0,w,Z)\vdash^*(q_1,\tau y_1,U_r(\tau x_1)Z)\boxed{=}$. Рассмотрим
%блять, снова
случаи:\begin{enumerate}
\item $|\tau|=0,|x_1|=0\Rightarrow|x|=0$~--- \underline{противоречие}
